window.addEventListener("load",function(){let canvas=document.getElementById("acid")
...
this.get=function(x,y,z,t){let n=[0,0,0,0]
let _n=0
var i=0
while(i<5){let osc=config.osc[i]
...
break
case "dyn":if(n[0]>this.dynamics.d){this.dynamics.d=n[0]}
...
if(t!=this.dynamics.t){this.dynamics.t=t
this.dynamics.d=0}
break}
return n}
...
this.render=function(){if(!this.stopped){setTimeout(function(){requestAnimationFrame(function(){this.render()}.bind(this))}.bind(this),60000/(config.settings.framerate*60));}
gl=this.gl
let colors=[]
let vertices=[]
var verticeN=0
...
while(y<this.dimensions.height){while(x<this.dimensions.width){if(feedback){relX=x/this.dimensions.width
centeredRelX=relX<=config.render.feedback.centerX?relX/config.render.feedback.centerX:1-((relX-config.render.feedback.centerX)/(1-config.render.feedback.centerX))
...
else{rgb=this.mothership.get(x+window.mousePosition[0],y+window.mousePosition[1],0,time)
...
this.upCount()}
this.init=function(){this.scrollContainer=document.getElementById("container")||false
let gl=this.gl
